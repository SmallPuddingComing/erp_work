...
class OtherWarehousOutOp(ControlEngine):
    def __init__(self):
    ...
    def check_invoice_number(self, invoice_number):
        ...
        return check_number_exists(self.controlsession,
                                   WarehouseOutOtherInvoice.__tablename__,
                                   'invoice_number',
                                   invoice_number)

    def create_other_warehouse_number(self, is_red_invoice_flag=None):
        """
        ...
        if is_red_invoice_flag:
            invoice_number = create_number('OROU{DATE}{SERIAL_NUMBER_4}',
                    self.controlsession,
                    WarehouseOutOtherInvoice.__tablename__,
                    'invoice_number',
                    is_auto_create=True)
        else:
            invoice_number = create_number('OBOU{DATE}{SERIAL_NUMBER_4}',
                    self.controlsession,
                    WarehouseOutOtherInvoice.__tablename__,
                    'invoice_number',
                    is_auto_create=True)
    ...
    def get_order_baseinfo(self, **kwargs):
        ...
        para_list = [invoice_number, start_time, stop_time, invoice_type_id, invoice_state_id, out_type]
        para_map_dict = {}
        ...
        if not None_list: # 表示未传入任何参数，搜索所有的数据
            ...
            for idx in useful_para_list:
                if idx == 0: # 单据单号搜索条件
                    rule.append(WarehouseOutOtherInvoice.invoice_number.cast(LargeBinary)==invoice_number)
                elif idx == 1: # 表示开始时间存在
                    s_date = datetime.strptime(start_time, "%Y-%m-%d").date()
                    rule.append(func.date_format(WarehouseOutOtherInvoice.invoice_date, '%Y-%m-%d') >= s_date)
                elif idx == 2: # 表示结束时间存在 （开始和结束时间必须同时存在）
                    e_date = datetime.strptime(stop_time, "%Y-%m-%d").date()
                    rule.append(func.date_format(WarehouseOutOtherInvoice.invoice_date, '%Y-%m-%d') <= e_date)
                elif idx == 3: # 表示单据类型（红蓝单据，对应了库存方向）
                    rule.append(WarehouseOutOtherInvoice.inventory_direction_id.cast(LargeBinary)==invoice_type_id)
                elif idx == 4: # 表示单据状态
                    rule.append(WarehouseOutOtherInvoice.invoice_state_id.cast(LargeBinary)==invoice_state_id)
                elif idx == 5: # 表示其他出库出库类型
                    rule.append(WarehouseOutOtherInvoice.outwarehouse_type_id.cast(LargeBinary)==out_type)
            total = self.controlsession.query(func.count(WarehouseOutOtherInvoice.id)).filter(rule).scalar()

            if page is None: # 表示不需要分页
                invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(rule).order_by(
                    WarehouseOutOtherInvoice.id).all()
            else:
                start = (page - 1) * pagecount
                invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(rule).order_by(
                    WarehouseOutOtherInvoice.id).limit(pagecount).offset(start)
            if invoice_datas:
    ...
    def get_all_invoice_baseinfo(self, page=None, pagecount = None):
        ...
        if page is None and pagecount is None:
            order_datas = self.controlsession.query(WarehouseOutOtherInvoice).order_by(WarehouseOutOtherInvoice.id).all()
            ...
            if page <= 0 or pagecount <= 0:
                raise ValueError("page and pagecount both must be None or both positive integer")
            start = (page - 1) * pagecount
            order_datas = self.controlsession.query(WarehouseOutOtherInvoice).order_by(WarehouseOutOtherInvoice.id
                                                                          ).limit(pagecount).offset(start)

        total = 0
        order_datas_list = []
        if order_datas:
            total = self.controlsession.query(func.count(WarehouseOutOtherInvoice.id)).scalar() # 记录总数
    ...
    def get_invoice_baseinfo_by_invoice_number(self, invoice_number=None):
        ...
        if invoice_number is None:
            order_data = self.controlsession.query(WarehouseOutOtherInvoice).all()
        else:
            order_data = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                WarehouseOutOtherInvoice.invoice_number.cast(LargeBinary)==invoice_number).first()
    ...
    def get_invoice_materialinfo_by_invoice_id(self, invoice_id=None):
        ...
        if invoice_id is None:
            raise CodeError(ServiceCode.service_exception, msg=u"单据单号ID是必传参数")
        material_datas = self.controlsession.query(WarehouseOutOtherDetail).filter(
            WarehouseOutOtherDetail.invoice_id==invoice_id).all()
        if not material_datas:
            raise CodeError(ServiceCode.service_exception, msg=u"单据单号ID为%s的物料信息记录不存在" % invoice_id)
        else:
            data_list = []
            ...
            for data in material_data_list:
                ...
                if search_flag != 0:
                    raise CodeError(ServiceCode.service_exception, msg=baseinfo_datas)
                ...
                select_data = self.select_invoice_associated_material(baseinfo_dict)
    ...
    def get_red_or_blue_invoice_baseinfo(self, red_blue_flag=None):
        ...
        if red_blue_flag is None:
            ...
            invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                WarehouseOutOtherInvoice.inventory_direction_id.cast(LargeBinary)==blue_flag).order_by(
                WarehouseOutOtherInvoice.id).all()
        elif red_blue_flag == red_flag: # 红字单据
            invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                WarehouseOutOtherInvoice.inventory_direction_id.cast(LargeBinary)==red_flag).order_by(
                WarehouseOutOtherInvoice.id).all()
        else:
            raise CodeError(ServiceCode.service_exception, msg=u"判别红蓝单据的参数有误")
    ...
    def get_invoice_baseinfo_by_state(self, invoice_state_type=None):
        ...
        if invoice_state_type is None: # 表示查询所有单据状态的单据记录
            invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).order_by(
                WarehouseOutOtherInvoice.id).all()
        else:
            if invoice_state_type not in (state_keys):
                ...
                invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                    WarehouseOutOtherInvoice.invoice_state_id.cast(LargeBinary)==invoice_state_type).order_by(
                    WarehouseOutOtherInvoice.id).all()
        invoice_data_list = []
        for data in invoice_datas:
            invoice_dict = self.paras_invoice_fields(data)
    ...
    def get_invoice_baseinfo_by_outtype(self, warehouse_out_type=None):
        ...
        """
        warehouse_out_keys = WarehouseManagementAttr.OTHER_OUT_WAREHOUSE_TYPE.keys()
        if warehouse_out_type is None: # 查询所有出库类型的单据记录信息
            ...
            if warehouse_out_type not in warehouse_out_keys:
                ...
                invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                    WarehouseOutOtherInvoice.outwarehouse_type_id.cast(LargeBinary)==warehouse_out_type).order_by(
                    WarehouseOutOtherInvoice.id).all()
        invoice_data_list = []
        for data in invoice_datas:
            invoice_dict = self.paras_invoice_fields(data)
    ...
    def get_invoice_baseinfo_by_date(self, start_date=None, stop_date=None):
        ...
        try:
            if start_date is None and stop_date is None:
                ...
                invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                    func.date_format(WarehouseOutOtherInvoice.invoice_date, '%Y-%m-%d') >= s_date,
                    func.date_format(WarehouseOutOtherInvoice.invoice_date , '%Y-%m-%d')<= e_date).\
                    order_by(WarehouseOutOtherInvoice.id).all()
            else:
                s_date = datetime.strptime(start_date, "%Y-%m-%d").date()
                invoice_datas = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                    func.date_format(WarehouseOutOtherInvoice.invoice_date, '%Y-%m-%d') == s_date).\
    ...
    def get_invoice_state(self, invoice_number=None):
        ...
        invoice_state_id = self.controlsession.query(WarehouseOutOtherInvoice.invoice_state_id).filter(
            WarehouseOutOtherInvoice.invoice_number.cast(LargeBinary)==invoice_number).first()

        if invoice_state_id is None:
            raise ValueError("%s does not exist in the table %s's invoice_number field" %
                             (invoice_number, WarehouseOutOtherInvoice.__tablename__))
        else:
    ...
    def get_invoice_id(self, invoice_number):
        ...
        invoice_id = self.controlsession.query(WarehouseOutOtherInvoice.id).filter(
            WarehouseOutOtherInvoice.invoice_number.cast(LargeBinary)==invoice_number).first()
    ...
    def get_customer_id(self, invoice_number):
        ...
        customer_id = self.controlsession.query(WarehouseOutOtherInvoice.customer_id).filter(
            WarehouseOutOtherInvoice.invoice_number.cast(LargeBinary)==invoice_number).first()
    ...
    def del_invoice_info_by_invoice_number(self, invoice_number):
        ...
        commit_flag = customer_op.del_customer_links([customer_id], WarehouseOutOtherInvoice.__tablename__)
        if not commit_flag:
        ...
        try:
            ...
            if commit_flag:
                customer_op.add_customer_links([customer_id], WarehouseOutOtherInvoice.__tablename__)
            raise
    ...
    def paras_invoice_fields(self, data_object):
        ...
        warehouse_op = GetWarehouseInfo()
        if isinstance(data_object, WarehouseOutOtherInvoice):
            ...
            data['invoice_type'] = WarehouseManagementAttr.INVOICE_TYPE.get(data['invoice_type_id'])
            ...
            material_info = user_op.get_user_info(data['materialHandler_id'])
            data['material_handler'] = material_info.get('name') # 领料员
            ...
            data['outgoing_type'] = WarehouseManagementAttr.OTHER_OUT_WAREHOUSE_TYPE.get(data['outgoingType_id']) # 其他出库出库类型
            customer_info = customer_op.get_customer_id_info(data['customer_id'])
            data['customer'] = customer_info.get("customer_name") if customer_info else "" # 客户
            ...
            if data.get('department_id') is None or not data.get('department_id'):
                data['material_department'] = ""
            else:
                department_info = user_op.get_department_name_byID(data['department_id'])
                data['material_department'] = department_info # 领料部
            return data
        else:
    ...
    def del_invoice_basedata(self, invoice_number, is_commit=False):
        ...
        """
        try:
            obj = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                WarehouseOutOtherInvoice.invoice_number.cast(LargeBinary)==invoice_number).first()
    ...
    def del_invoice_detail_data(self, invoice_id):
        ...
        op = MaterialWarehouseInfoOp()
        objs = self.controlsession.query(WarehouseOutOtherDetail).filter(
            WarehouseOutOtherDetail.invoice_id==invoice_id).all()
        link_dict = {}
        for obj in objs:
            try:
        ...
        try:
            ...
            if base_commit_flag:
                ...
                material_base_op.add_operate_record(uid=session['user']['id'],
                                                    ...
                                                    tablename=WarehouseOutOtherInvoice.__tablename__
                                                    )
                # 修改客户操作记录
                commit_flag = customer_op.add_customer_links([invoice_data_dict.get("customer_id")],
                                                WarehouseOutOtherInvoice.__tablename__)
                if not commit_flag:
                    raise CodeError(ServiceCode.service_exception, msg=u"客户操作记录提交失败")
                self.controlsession.commit()
            ...
            self.controlsession.rollback()
            if commit_flag:
                customer_op.del_customer_links([invoice_data_dict.get("customer_id")],
                                                WarehouseOutOtherInvoice.__tablename__)
            if base_commit_flag:
    ...
    def update_invoice_basedata(self, basedata_dict):
        try:
            ...
            self.__save_check_basedata(basedata_dict)
            obj = self.controlsession.query(WarehouseOutOtherInvoice).filter(
                WarehouseOutOtherInvoice.invoice_number.cast(LargeBinary)==basedata_dict.get("invoice_number")).first()
    ...
    def get_invoice_operate_record(self, invoice_number, page=1, per_page=5):
        ...
        record_datas, total = oper_op.get_record(start, per_page, WarehouseOutOtherInvoice.__tablename__, invoice_id)
        record_list = []
    ...
    def select_invoice_associated_material(self, material_dict):
        return_data = {}
        return_data['material_id'] = material_dict.get('material_id')
        return_data['material_code'] = material_dict.get('material_code')
        return_data['material_name'] = material_dict.get('material_name')
        return_data['specification_model'] = material_dict.get('specification_model')
        return_data['unit'] = material_dict.get('unit')
        return_data['stock_number'] = material_dict.get('stock_number')
        return_data['deliver_warehouse'] = material_dict.get('deliver_warehouse')

        return_data['warehouse_id'] = material_dict.get('warehouse_id')
        return_data['inventory_number'] = material_dict.get('inventory_number')
        return_data['is_nondefective'] = material_dict.get('is_nondefective')
    ...
    def __save_check_basedata(self, m_dict):
        ...
        all_customer_id = [m_dict.get('customer_id')]
        ...
        assert base_op.check_other_invoice_type(m_dict.get('invoice_type_id')), \
            "invoice type error: %s is wrong for the invoice" % m_dict.get('invoice_type_id')
        assert base_op.check_other_out_type(m_dict.get('outgoingType_id')), \
    ...
    def commit_invoice_data(self, invoice_number):
        ...
        try:
            ...
            invoice = self.controlsession.query(WarehouseOutOtherInvoice).filter_by(invoice_number=invoice_number).first()
