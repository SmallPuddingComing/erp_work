...
from data_mode.erp_supply.mode.material_repertory_model.warehousein_other_detail import WarehouseInOtherDetail
from data_mode.erp_supply.mode.material_repertory_model.warehousein_other_invoice import WarehouseInOtherInvoice
...
class OtherInWarehouseOp(ControlEngine):
    """
    外购入库op
    """
    def __init__(self):
    ...
    def get_other_list_info(self, page, per_page, search_dict=None):
        """
        ...
        :param search_dict:{
            ...
            other_storage_type_id:      #其它入库类型id 没有不传
        }
        :return:
        """
        try:
            ...
            if search_dict is not None:
                ...
                if invoice_type_id is not None and invoice_type_id is not "" and invoice_type_id is not u"":
                    condition.append(WarehouseInOtherInvoice.inventory_direction_id == invoice_type_id)
                ...
                if invoice_state_id is not None and invoice_state_id is not "" and invoice_state_id is not u"":
                    condition.append(WarehouseInOtherInvoice.invoice_state_id == invoice_state_id)
                    print "condition1111:           ", condition

                other_storage_type_id = search_dict.get("other_storage_type_id")
                if other_storage_type_id is not None \
                        and other_storage_type_id is not "" and \
                        other_storage_type_id is not u"":
                    condition.append(WarehouseInOtherInvoice.inwarehouse_type_id == other_storage_type_id)

            print "condition:           ", condition
            rs = self.controlsession.query(WarehouseInOtherInvoice).\
                filter(condition).limit(per_page).offset(start).all()
            total = self.controlsession.query(func.count(WarehouseInOtherInvoice.id)).filter(condition).scalar()
            invoice_info_list = []
            ...
            for item in rs:
                ...
                invoice_dict["invoice_state"] = WarehouseManagementAttr.INVOICE_STATE.get(item_data["invoice_state_id"])
                # 其它入库类型
                invoice_dict["other_storage_type_id"] = item_data["other_storage_type_id"]
                invoice_dict["other_storage_type"] = WarehouseManagementAttr.OTHER_INWAREHOUSE_TYPE.\
                    get(item_data["other_storage_type_id"])
    ...
    def get_crate_other_info(self, invoice_type_id):
        """
        获取其它入库新建页所需数据
        ...
        try:
            ...
            invoice_dict["invoice_number"] = create_number("PBIN{DATE}{SERIAL_NUMBER_4}",
                                                           ...
                                                           table_name=WarehouseInOtherInvoice.__tablename__,
                                                           field="invoice_number")

            # 获取日期
            invoice_dict["invoice_date"] = datetime.datetime.now().strftime("%Y-%m-%d")
            # 制单人
            invoice_dict["maker"] = session["user"]["name"]
            invoice_dict["maker_id"] = session["user"]["id"]
            # 单据类型
            invoice_dict["invoice_type_id"] = WarehouseManagementAttr.OTHER_IN_WAREHOUSE
    ...
    def get_copy_other_info(self, invoice_number):
        """
        获取其它入库复制页面所需数据
        ...
        try:
            ...
            if invoice_number is not None:
                rs_invoice = self.controlsession.query(WarehouseInOtherInvoice).filter(
                    WarehouseInOtherInvoice.invoice_number == invoice_number
                ).first()
                if rs_invoice:
                    ...
                    # 业务员
                    if invoice_data["salesman_id"] is not None and invoice_data["salesman_id"] is not "":
                        rs_user = user_op.get_user_info(invoice_data["salesman_id"])
                        invoice_dict["salesman"] = rs_user["name"]
                        invoice_dict["salesman_id"] = invoice_data["salesman_id"]
                    else:
                        invoice_dict["salesman"] = ""
                        invoice_dict["salesman_id"] = ""
                    ...
                    # 其它入库类型
                    if invoice_data["other_storage_type_id"] is not None \
                            and invoice_data["other_storage_type_id"] is not "":
                        invoice_dict["other_storage_type_id"] = invoice_data["other_storage_type_id"]
                        invoice_dict["other_storage_type"] = WarehouseManagementAttr.OTHER_INWAREHOUSE_TYPE[
                                                                    invoice_dict["other_storage_type_id"]
                                                                    ]
                    else:
                        invoice_dict["other_storage_type_id"] = ""
                        invoice_dict["other_storage_type"] = ""
                    # 供应商
                    ...
                    rs_detail = self.controlsession.query(WarehouseInOtherDetail).filter(
                        WarehouseInOtherDetail.invoice_id == rs_invoice.id).all()
                    detail_list = []
                    for item in rs_detail:
                        item_dict = item.to_json()
                        print "material_id:   ", item_dict["material_id"]
    ...
    def get_update_other_info(self, invoice_number):
        """
        获取其它入库修改页面所需数据
        :param invoice_number:
        :return:
        ...
        try:
            ...
            if invoice_number is not None:
                ...
                if rs_invoice:
                    ...
                    #
                    if invoice_data["invoice_state_id"] == "13-1":
                        invoice_data["state_type"] = 1
                    elif invoice_data["invoice_state_id"] == "13-2":
                    ...
                    if invoice_data["other_storage_type_id"] is not None \
                            and invoice_data["other_storage_type_id"] is not "":
                        invoice_data["other_storage_type"] = WarehouseManagementAttr.OTHER_INWAREHOUSE_TYPE[
                                                                    invoice_data["other_storage_type_id"]
                                                                    ]
                    else:
                        invoice_data["other_storage_type"] = ""
                        invoice_data["other_storage_type_id"] = ""
                    # 供应商
                    rs_supplier = supplier_op.get_supplier_base_info(invoice_data["supplier_id"])
    ...
    def get_invoice_operate_record(self, invoice_number, page=1, per_page=5):
        ...
        rs = self.controlsession.query(WarehouseInOtherInvoice).filter_by(invoice_number=invoice_number).first()
        if rs:
            invoice_id = rs.id
        else:
        ...
        record_datas, total = oper_op.get_record(start, per_page, WarehouseInOtherInvoice.__tablename__, invoice_id)
        record_list = []
    ...
    def save_other_info(self, invoice_info):
        """
        保存外购入库单据信息
        :param invoice_info:
        {
            ...
            other_storage_type_id: str		# 出库类型ID
            ...
            rows:[		#明细信息
            {
            product_id: int		# 产品ID（即物料ID）
            warehouse_id: int	# 发货仓库ID
            realHair_number: int		# 实发数量
            is_nondefective_flag: int		# 检验是否为良品(1:是 0:否)
            }
            ]
        ...
        try:
            ...
            # 查询单据id，然后保存详细信息
            rs_invoice = self.get_other_info_by_number(invoice_info["invoice_number"])
            ...
            material_base_op.add_operate_record(uid=session['user']['id'],
                                                ...
                                                is_commit=False,
                                                db_session=self.controlsession,
                                                tablename=WarehouseInOtherInvoice.__tablename__
                                                )
            self.controlsession.commit()
            pass
        except CodeError:
            rs = self.controlsession.query(WarehouseInOtherInvoice).filter(
                WarehouseInOtherInvoice.invoice_number == invoice_info["invoice_number"]
            ).first()
            if rs:
    ...
    def update_other_info(self, invoice_info):
        """
        修改其它入库信息
        :param invoice_info:
        :return:
        ...
        try:
            ...
            rs_invoice = self.controlsession.query(WarehouseInOtherInvoice).filter_by(
                    invoice_number=invoice_info["invoice_number"]).first()
            print "111111111", rs_invoice.invoice_state_id
            if not rs_invoice:
                ...
                # 先把原来的绑定的物料信息删除掉
                rs_detail = self.controlsession.query(WarehouseInOtherDetail).\
                ...
                rs_invoice.inwarehouse_type_id = invoice_info["other_storage_type_id"]
                self.controlsession.add(rs_invoice)

                for item in invoice_info["rows"]:
                    item_obj = WarehouseInOtherDetail(invoice_id=rs_invoice.id,
    ...
    def commit_other_info(self, invoice_data_dict):
        """
        外购入库提交更新库存
    ...
    def other_invoice_delete(self, invoice_number):
        """
        删除外购入库单据
        ...
        try:
            ...
            if rs:
                ...
                if invoice_info["invoice_state_id"] == "13-2":
    ...
    def check_other_invoice_number(self, invoice_number):
        """
        单据单号唯一性检查
        :param invoice_number:
        :return:
        ...
        return check_number_exists(self.controlsession,
                                   WarehouseInOtherInvoice.__tablename__,
                                   'invoice_number',
                                   invoice_number)
    ...
    def save_invoice_info(self, invoice_info, is_commit=False):
        ...
        """
        try:
            # 时间只能根据后台生成 不用前端传上来的时间
            invoice_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            r_obj = WarehouseInOtherInvoice(invoice_number=invoice_info["invoice_number"],
                                            ...
                                            inwarehouse_type_id = invoice_info["other_storage_type_id"],
    ...
    def save_detail_info(self, invoice_id, detail_info, is_commit=False):
        ...
        try:
            ...
            # for item in detail_info:
                item_obj = WarehouseInOtherDetail(invoice_id=invoice_id,
    ...
    def get_other_info_by_number(self, invoice_number):
        """
        更具单据单号查询单据信息
        :param invoice_number:
        :return:
        ...
        try:
            rs = self.controlsession.query(WarehouseInOtherInvoice).filter(
                WarehouseInOtherInvoice.invoice_number == invoice_number).first()
            if rs:
                r_data = rs.to_json()
            else:
